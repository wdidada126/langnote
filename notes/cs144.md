...
【Stanford CS144 计算机网络 工具篇1：telent SSH 远程登录工具-哔哩哔哩】 https://b23.tv/aajEjz2

那个老哥的博客在这里（希望这种不算侵权啥的吧）
...
https://github.com/edidada/simple-db-hw-2021
...
我在make check_webget的时候报没有权限的错怎么解决呀，我明明用了sudo或者root用户了呀。
:~/sponge/build$ sudo make check_webget
Scanning dependencies of target check_webget
【100%】 Testing webget...
Test project /home/fhkong/sponge/build
    Start 31: t_webget
Process not started
 /home/fhkong/sponge/tests/webget_t.sh
【permission denied】
1/1 Test #31: t_webget .........................***Not Run   0.00 sec

还有就是我试过将webget_t.sh文件修改为可执行，然后报了找不到文件的错...

find . \( -name build -o -name .git \) -prune -o -type f -exec dos2unix -s {} \;
...
https://github.com/edidada/CS144_Spring_2023
...
标志位 全称  核心作用    使用场景
ACK Acknowledgment  确认收到数据  几乎每个包（除了第一个 SYN 包）都会置 1。
SYN Synchronize 同步序号，发起连接   三次握手的前两步。
FIN Finish  结束连接，优雅关闭   四次挥手时使用。
RST Reset   重置连接，强制中断   连接出错、拒绝访问时使用。
PSH Push    推送数据，立即交付给应用    交互式应用（如 SSH、Telnet）。
URG Urgent  紧急指针有效  很少用，已被应用层优先级替代。
