...
debian xmake包做到一半，发现xmake依赖的库tbox libsv啥的都已经在debian仓库里了。
sudo apt install libtbox-dev可以直接装了
...
需记忆的概念
读放大（Read Amplification），写放大（Write Amplification）空间放大（Space Amplification）
...
为了模拟真实的线上场景，我们通过 TCPCopy，把线上实际的查询流量引到测试环境，先后经过 13 轮的性能测试，最终6个MySQL库相对一个 Oracle，在当时的数据量下，SQL执行时间基本持平。
