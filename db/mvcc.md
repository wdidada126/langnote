...
## 用 Coq、Iris、Perennial 验证 vMVCC：概念、用途与边界

原段落中的“vMVCC 使用 Perennial、Iris 和 Coq 进行验证”是正确的，但三者不是同一种东西，更不是 vMVCC 的运行时依赖。它们从下到上构成形式化验证工具链；vMVCC 才是被验证的 Go 事务库。

```text
Coq / Rocq
    -> 交互式定理证明器：定义、证明项和内核检查
Iris
    -> 在 Coq/Rocq 中实现的高阶并发分离逻辑：不变式、所有权、ghost state、逻辑原子性
Perennial + Goose
    -> 在 Iris 之上验证并发/崩溃安全 Go 系统；Goose 将受支持的 Go 代码嵌入证明语言
vMVCC
    -> 被验证的高性能 Go 键值事务库，采用 MVCC 实现严格可串行化事务语义
```

### 四个概念分别是什么

| 概念 | 它是什么 | 在 vMVCC 中的用途 | 它不是什么 |
| --- | --- | --- | --- |
| Coq（现项目生态也常称 Rocq） | 交互式定理证明器与依赖类型语言；开发者写定义、引理和证明，由小型内核检查证明项 | 机械检查 vMVCC 的规范、程序逻辑规则和所有证明，避免“论文推导看起来对但细节漏掉” | 不是数据库、不是运行时事务引擎，也不是自动测试工具。 |
| Iris | 在 Coq/Rocq 中机械化的高阶并发分离逻辑框架 | 描述并发线程如何拥有/共享内存资源；用不变式、ghost state、逻辑原子三元组和预言变量证明细粒度并发算法 | 不是 Go 编译器，也不会直接把任意程序自动证明正确。 |
| Perennial | 基于 Iris 的系统验证框架，面向并发、存储和分布式系统，扩展了崩溃 weakest precondition、崩溃不变式、幂等与 refinement 等工具 | 为 Go 系统提供可验证的程序逻辑与基础库；vMVCC 的证明位于其代码库的 `program_proof/mvcc` | 不是生产数据库框架；“Perennial 支持崩溃安全推理”不等于其中每个项目都已证明崩溃恢复。 |
| Goose | Go 到 Perennial 证明语言的翻译工具与语义桥梁 | 将 vMVCC 的受支持 Go 子集转换为可在 Coq/Iris/Perennial 中推理的 `goose_lang` 程序 | 不是通用 Go 编译器；Go 语言、外部库和系统调用必须落在其建模支持范围内。 |

分离逻辑的直觉是：证明上下文不只记录“命题为真”，也记录“我拥有哪一块资源”。例如 `x |-> v` 可理解为“当前线程有权访问地址 `x`，其中值为 `v`”。并发场景中，独占资源可随锁或线程转移，共享资源由不变式保护。Iris 的价值在于把这些推理组合成可复用的高层规格，而不是在每个函数证明中手工展开所有共享内存细节。

### vMVCC 实际验证了什么

vMVCC 是 OSDI 2023 的研究原型，一个高性能、内存中的事务性键值库。它为每个事务分配严格递增的时间戳，读操作从小于该时间戳的最新版本取得值；写操作先存入事务私有 write buffer，提交时锁定待修改 tuple、检查冲突并追加版本。旧版本在不再可能被活跃或未来事务读取后由后台 GC 回收。

```text
Begin()       -> 分配严格递增的 transaction timestamp
Read(key)     -> 先看本地 write buffer；否则读取 timestamp 之前的最新版本
Write/Delete  -> 只改本地 write buffer
Commit()      -> 加锁并检查 tslast/冲突；成功则原子写入新版本，否则 abort
Abort()       -> 丢弃本地 write buffer
GC            -> 根据活跃事务及未来事务的安全时间下界删除过期版本
```

论文用逻辑原子规格表达 `db.Run`：从客户端观察，每个成功事务似乎在某个线性化点一次性读取和修改抽象数据库状态，因此可视为严格可串行化语义的程序逻辑形式化。这个结论比“每个函数没有数据竞争”强得多，因为它还约束了并发事务对应用可见的结果。

但它也有清楚的范围：vMVCC 不是 PostgreSQL/MySQL 的完整实现，不证明 SQL、查询优化、二级索引、WAL、磁盘持久化、复制、分片、网络故障恢复、访问控制或业务不变量。它证明的是给定建模假设和所验证 Go 代码下的事务并发核心。将 vMVCC 的结果直接外推成“任意使用 MVCC 的生产数据库已正确”是错误的。

### 为什么 MVCC 的证明特别困难

普通锁式事务可以把线性化点放在一次显式加锁或提交操作上。vMVCC 的事务却可能先读旧版本，再在别的线程推进时间戳时获得其抽象效果；某一事务的逻辑线性化点甚至不必对应它自身执行的某一行 Go 代码。证明需要同时维持：

1. **版本可见性**：读取的版本恰好是事务时间戳之前最后一个有效版本，不读到不该见的数据。
2. **冲突规则**：较早时间戳的写事务若发现更晚事务已读/写同一 tuple，必须失败，否则无法维持可串行化观察。
3. **时间戳单调性**：RDTSC 只是硬件计数来源；vMVCC 还用 site ID、每 site 互斥锁和等待循环，使生成的交易 ID 严格递增。简单地“调用 RDTSC”并不足以得到这个性质。
4. **GC 安全性**：回收的版本不能被任一已开始事务、或时间戳可能落入该版本有效区间的未来事务读取。
5. **物理状态与逻辑状态的对应**：版本链、锁、活动事务表等实现细节必须始终对应抽象的序列化数据库视图。

### 预言变量和 ghost state 的直觉

预言变量（prophecy variable）是**证明中的幽灵状态**，编译后不会存在于 Go 程序或数据库中。它允许证明在尚未看到未来非确定性结果时，先把“未来某次操作会产生什么结果”的逻辑承诺放入不变式，等执行真正发生后再验证承诺一致。vMVCC 用它处理“事务体的读写结果”与“事务在逻辑时间线上的原子效果”并不在同一个物理代码点出现的问题。

这不是运行时预测，也不是给事务预先算出结果。若没有这种证明工具，开发者仍可实现同样的 MVCC；只是很难模块化地证明逻辑原子性。Iris 的 ghost state 同理，只服务于证明中维护抽象版本表、slot 和不变式，不增加生产运行时开销。

### 对数据库工程的实际价值

- **设计审查**：把“读到哪个版本、何时提交、冲突何时失败、GC 何时安全”写成可执行前置条件和不变式，比仅凭压测更容易发现边界错误。
- **高风险并发组件**：适合事务管理器、锁/无锁索引、日志、缓存一致性协议、复制状态机、元数据服务等规模较小但正确性要求极高的核心组件。
- **回归保障**：证明会迫使接口明确语义；实现优化或重构后，无法重新建立证明往往表示某个并发假设已经被破坏。
- **局限与成本**：证明工程量大。vMVCC 论文报告约 827 行 Go 实现对应约 11,117 行 Coq 证明（不含为验证所做的 Perennial 改动），因此它适合关键算法与基础设施，不是每个 CRUD 服务都需要的默认手段。

### 面向 Java/数据库开发者的学习顺序

1. 先掌握 MVCC 的普通工程语义：快照、版本可见性、写冲突、GC、安全时间点和严格可串行化/快照隔离的差别。
2. 学 Coq/Rocq 的最小单元：归纳类型、命题、量词、等式、函数规格、交互式证明和“内核检查”与单元测试的区别。
3. 学分离逻辑：堆所有权、frame rule、锁不变式；再阅读 Iris Tutorial，理解弱前置条件、invariant、ghost state 和逻辑原子规格。
4. 阅读 vMVCC 论文第 2 节的算法，再读第 3-4 节的规格与证明思路；先理解为何要证明，而不是一开始钻入 Coq tactic。
5. 最后检出 Perennial，先运行已有证明，再从 `program_proof/mvcc` 的 tuple、transaction、timestamp 子模块向上追踪。修改实现后应先写/改规格和引理，绝不能只看 Go 测试通过就声称保持证明。

### 参考资料

- vMVCC 项目与论文、代码、证明入口：<https://pdos.csail.mit.edu/projects/vmvcc.html>
- OSDI 2023 论文《Verifying vMVCC, a high-performance transaction library》：<https://iris-project.org/pdfs/2023-osdi-vmvcc.pdf>
- Perennial 源码、目录说明和 vMVCC 证明位置：<https://github.com/mit-pdos/perennial>
- Perennial 的并发/崩溃安全 Go 系统验证简介：<https://www.chajed.io/perennial/>
- Iris 项目、教程和论文：<https://iris-project.org/>
