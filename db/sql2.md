...
数据库 SQL 中的窗口函数可以按照以下方式进行分类：

排名函数：排名函数用于计算每个行的排名值，通常与 ORDER BY 子句一起使用。以下是一些常见的排名函数：

RANK()：为每个行分配一个递增的排名值，根据 ORDER BY 子句中指定的列进行排序。如果在同一排序组中有多个行具有相同的排序值，则这些行将获得相同的排名，下一个排名将跳过相应的行数。

DENSE_RANK()：类似于 RANK()，但它在具有相同排序值的行之间分配连续的排名。即使有多个行具有相同的排序值，也会为它们分配下一个连续的排名。

NTILE()：将行分配给指定数量的组，并为每个行分配一个组号，根据 ORDER BY 子句中指定的列进行排序。
ROW_NUMBER()：为每个行分配一个唯一的数字，根据 ORDER BY 子句中指定的列进行排序。

聚合函数：聚合函数用于计算一组行的统计值，例如总和、平均值、计数等。以下是一些常见的聚合函数：
SUM()：计算指定列的总和。
AVG()：计算指定列的平均值。
COUNT()：计算行数。
MAX()：找出指定列的最大值。
MIN()：找出指定列的最小值。
STDEV()、STDEVP()：计算指定列的标准偏差。
VAR()、VARP()：计算指定列的方差。
窗口函数：窗口函数用于计算与当前行相关联的行的聚合值或计算其他行的累计值。以下是一些常见的窗口函数：
RANK() OVER (PARTITION BY ... ORDER BY ...)：在给定分区内，根据 ORDER BY 子句中指定的列对行进行排名，并返回每行的排名值。
DENSE_RANK() OVER (PARTITION BY ... ORDER BY ...)、NTILE(n) OVER (PARTITION BY ... ORDER BY ...)：类似于 RANK() OVER，但它们会为具有相同排序值的行分配连续的排名或组号。
SUM() OVER (PARTITION BY ... ORDER BY ...)、AVG() OVER (PARTITION BY ... ORDER BY ...)、COUNT() OVER (PARTITION BY ... ORDER BY ...)：在给定分区内，根据 ORDER BY 子句中指定的列对行进行聚合计算，并返回每行的聚合值。
LAG()、LEAD()：用于访问前一行或下一行的值，可以指定偏移量和排序列。
SUM() OVER (ORDER BY ... ROWS BETWEEN ...)、SUM() OVER (ORDER BY ... ROWS UNBOUNDED PRECEDING) 等：用于计算累计值或其他窗口函数的值的累计结果。
