...
在上述示例中，我们使用 `@Autowired` 注解将 `javax.validation.Validator` 实例注入到 Spring Bean 中，并在 `savePerson` 方法中使用它来验证 `Person` 对象。如果存在验证错误，我们可以根据需要进行处理。

总之，以上是使用 `validation-api` 对 Spring IoC 容器中的 Bean 进行校验的基本步骤。你可以根据实际情况进行调整和扩展。