...
TiDB 8.5.0 是一个重要的长期支持版本（LTS），它在性能、扩展性、稳定性以及可观测性等多个方面都有显著的改进和新特性。以下是基于你提到的 TiDB 版本（8.0.11-TiDB-v8.5.0）的一些关键新特性和增强功能：

性能与扩展性
并行排序：从 v8.2.0 开始引入，通过并行化排序过程来提高查询性能。
优化 KV 请求批处理策略：从 v8.3.0 开始引入，旨在减少网络交互次数，提高数据处理效率。
并行获取 TSO：从 v8.4.0 开始引入，提高了分布式事务的时间戳服务（TSO）获取效率。
降低 DELETE 操作的资源开销：减少了执行 DELETE 操作时所需的系统资源。
优化缓存表场景性能：提升了使用缓存表时的整体性能。
Hash Join 算法优化：作为实验特性从 v8.4.0 开始引入，进一步提升了特定类型查询的性能。
存储层改进
TiKV MVCC 内存引擎：引入了内存引擎来缓存最新的 MVCC 数据版本，有助于快速访问最新数据，特别是在频繁更新或历史版本保留时间较长的场景下。
分布式框架与资源管理
实例级执行计划缓存：作为实验特性从 v8.4.0 开始引入，允许同一个 TiDB 实例的所有会话共享执行计划缓存，减少SQL编译时间，提升OLTP性能。
分区表全局索引：正式成为功能，支持更高效的非分区列检索，消除了唯一键必须包含分区键的限制。
默认允许将 Projection 算子下推到存储引擎：通过分散负载到存储节点，减少节点间的数据传输，提升数据库性能。
可观测性与诊断能力
日志标识和关联：通过注入会话别名，简化了事务端到端追踪的过程。
索引选择观测：通过新的系统表可以识别并删除无用或低效的索引。
Runaway Queries 支持更多触发条件，并能够切换资源组：提供更加精确的查询控制，保障业务稳定性。
安全性
快照备份数据和日志备份数据支持客户端加密：确保数据在存储和传输过程中的安全性。
其他特性
SQL 外键约束：正式支持外键约束，用于建立表之间的关联关系，保证数据的一致性和完整性。
向量搜索功能：作为实验特性引入，支持基于语义的搜索方法，适用于个性化推荐、欺诈检测等AI应用场景。
这些只是 TiDB 8.5.0 中的一部分改进和新增功能。随着版本的迭代，PingCAP 不断地对 TiDB 进行优化和增强，以满足用户对于性能、稳定性和易用性的需求。如果你正在考虑升级到这个版本，建议详细阅读官方发布的 Release Notes 来了解所有变更和改进建议。
...
PingCAP团队的论文《TiDB: A Raft-based HTAP Database》入选VLDB 2020 ，成为业界第一篇 Real-time HTAP 分布式数据库工业实现的论文。
...
你可以使用 TiDB 或者 TiSpark 读取 TiFlash，TiDB 适合用于中等规模的 OLAP 计算，而 TiSpark 适合大规模的 OLAP 计算，你可以根据自己的场景和使用习惯自行选择。
