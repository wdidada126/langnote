...
1.spring-Redis-date这个jar包自身存在缺陷（由于Redis协议需要，Redis的expire方法会将超时时间进行类型转换后传给Redis，Long型转Int类型并限制长度，超长会报错），导致超时时间的最大长度为2147483647L，研发人员**在使用Redis设置超时时间的时候需要注意：如果使用毫秒做单位，最大支持24.8天超时。经过测试可以通过设置超时时间单位为秒，或者升级使用Redis-date1.8.4+jedis2.9.0解决。**
